...
- **Product authority:** Confirmed brainstorm (2026-08-30). Grounded in shared patterns from portfolio and HyperActivity; does not fork either product.
...
Starting a new Nuxt app today means re-assembling conventions from portfolio and HyperActivity by hand (Pug, SCSS, env matrix, lint, agents, CI). This template makes that kit cloneable once so new apps start aligned without carrying product-domain weight.
...
- KD5. Always-on floor: tooling, agent rules, Pug, SCSS, CI; plus Node 26, HyperActivity-style `SYS_ENV`, and always-on Playwright. (session-settled: user-directed)
...
- KD8. Lean single-app layout (HyperActivity-like), not Nx monorepo. (session-settled: user-approved via Approach A)
...
- R4. Runtime env uses HyperActivity-style `SYS_ENV` ∈ `local` | `image` | `server` and Node lifecycle ∈ `development` | `test` | `production`.
...
- R12. Repo remains usable as a GitHub template without product/domain content from portfolio or HyperActivity.
...
- HyperActivity or portfolio product/strategy content
